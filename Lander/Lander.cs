...
namespace SpaceGame.Lander
{
    internal class Lander
    {
            ...
            LanderState landerState)
	    {
            ...
            // If the lander is docked, we need to get to the surface of the planet (altitude == 0)
            // Else, we need to get back to the ship (altitude == 1000)
            TargetAltitude = landerState == LanderState.Docked ? 0 : 1000;
            DistanceFromTarget = Math.Abs(TargetAltitude - StartingAltitude);
        }
        ...
        public LanderState RunLandingCycle(double currentFlowRate)
        {
            ...
            if (TotalFuel < 0)
            {
                ...
                this.LanderState = LanderState.OutOfFuel;
            }
            ...
            else if (DistanceFromTarget <= 0 && Velocity > MAX_COMPLETION_VELOCITY)
            {
                ...
                this.LanderState = LanderState.Crashed;
            }
            ...
            else if (DistanceFromTarget <= 0 && Velocity <= MAX_COMPLETION_VELOCITY)
            {
                ...
                this.LanderState = LanderState.Landed;
            }
        ...
        // Run the action of the lander landing on a planet
        public LanderState RunDockingCycle(double currentFlowRate)
        {
            double currentThrust = currentFlowRate * MaxThrust;    // Current thrust is current Flow Rate * Max Thrust
            if (!_altimeterIsSet)
            {
